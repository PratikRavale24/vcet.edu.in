    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">{items.length} achievement{items.length !== 1 ? 's' : ''}</p>
        </div>
        <Link to="/admin/achievements/new" className="flex items-center gap-2 bg-yellow-400 hover:bg-yellow-300 text-[#0A1128] font-bold px-4 py-2.5 rounded-xl text-sm transition-colors">
      ...
      <div className="bg-white/[0.03] border border-white/8 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-white/30">No achievements yet.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/8">
                <th className="text-left text-xs text-white/30 uppercase tracking-wider px-5 py-3 font-medium">Icon</th>
                <th className="text-left text-xs text-white/30 uppercase tracking-wider px-5 py-3 font-medium">Title</th>
                <th className="text-left text-xs text-white/30 uppercase tracking-wider px-5 py-3 font-medium">Value</th>
            ...
            <tbody className="divide-y divide-white/5">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-white">{item.title}</td>
                  <td className="px-5 py-3.5">
                    <span className="text-yellow-400 font-bold text-base">{item.value}</span>
                  </td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center justify-end gap-3">
                      <Link to={`/admin/achievements/${item.id}/edit`} className="text-white/40 hover:text-white text-xs transition-colors">Edit</Link>
